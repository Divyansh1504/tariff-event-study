...
**A result whose statistical significance flips based on eight months of routine price-adjustment
revisions was never a robust result.** If FF6-EW's alpha were real, its significance shouldn't
hinge on which day you happened to pull the data. The correct reading of this instability is
that it's additional evidence *against* the presence of real alpha, not a hint that the original
"significant alpha" claim was onto something after all — a stable null doesn't wobble across the
1.96 line on a data refresh; a fragile, noise-driven estimate does exactly that. Treat all four
t-stats as "not distinguishable from zero with any confidence," full stop — not as three misses
and one near-hit. See the full discussion in `notebooks/report2_portfolio_event_study.ipynb`.
...
- The core "did tariff exposure show up in returns" question is **not answered affirmatively** by
  ...
  effects clear conventional significance thresholds.
